...
class TestCaseSubmissionService {
    static async getTestCaseSubmissionById(id: number): Promise<TestCaseSubmissionResponseType> {
        try {
            const testCaseSubmission = await TestCaseSubmission.findByPk(id) as Model & TestCaseSubmissionResponseType;
    ...
    static async addTestCaseSubmission(testCaseId: number, submissionId: number, response: CheckCodeResponseType): Promise<TestCaseSubmissionResponseType> {
    ...
    static async updateTestCaseSubmission(id: number, response: CheckCodeResponseType) {
        try {
            const { output, error, status } = response;
            const submission = await TestCaseSubmission.findByPk(id) as Model & TestCaseSubmissionResponseType;
            if (!submission) {
    ...
    static async saveTestCaseSubmission(testCaseId: number, submissionId: number, response: CheckCodeResponseType) {
        try {
            const existingSubmission = await this.getTestCaseSubmissionByUserIdAndTestCaseId(submissionId, testCaseId);
            if (existingSubmission) {
                // Update existing submission
                return await this.updateTestCaseSubmission(existingSubmission.id, response);
            }

            // Add a new submission if it doesn't exist
            return await TestCaseSubmissionService.addTestCaseSubmission(testCaseId, submissionId, response);
        } catch (error) {
