...
const OfferTestCase: React.FC<OfferTestCaseProps> = ({
  testCaseNumber,
  testCase,
  isHidden,
  onChange,
  ...
  const handleGetOutput = async () => {
    ...
    if (output) {
      ...
      onChange(
        "status",
        error
          ? TestCaseSubmissionStatusEnum.FAILED
          : TestCaseSubmissionStatusEnum.COMPLETED,
      );
    } else if (error) {
  ...
  return (
    <div
      className={`w-full flex flex-col justify-center rounded-xl shadow-md bg-vscode dark:bg-vscode-dark transition-all duration-200 relative`}
    >
      <button
        onClick={handleToggleDetails}
        className="flex items-center px-2 py-4 justify-between w-full h-full rounded-xl hover:bg-neutral-100 dark:hover:bg-neutral-800 cursor-pointer select-none"
      >
        <div className="flex items-center gap-2 px-2 h-full">
          <StatusIcon className={cn("size-4", statusClassName)} />
          <p className="text-sm font-semibold">Test case #{testCaseNumber}</p>
          {isHidden && <p className="text-xs text-gray-500">Hidden</p>}
        </div>
      </button>
      ...
      {sending ? (
        <ClipLoader
          size={20}
          color="#1752F0"
          className="absolute top-4 right-2 size-4 text-primary"
        />
      ) : (
        <button
          onClick={handleGetOutput}
          className={`absolute top-4 right-2 rounded-full flex items-center justify-center border border-primary size-6 hover:bg-blue-100 dark:hover:bg-blue-900 cursor-pointer transition-colors`}
        >
        ...
        <div className="flex flex-col gap-2 px-4 pt-2 pb-4 w-full text-xs">
          ...
          <div className="flex flex-col gap-2 h-full">
            ...
            {testCase.output ? (
              ...
              <p className="text-neutral-400 dark:text-neutral-600 pb-2">
                No output provided.
              </p>
            )}
          </div>
          ...
          <Button
            className="w-full bg-red-500 dark:bg-red-900 hover:dark:bg-red-800 text-white hover:bg-red-600 mt-4 cursor-pointer"
            variant="destructive"
            size="sm"
            onClick={onDelete}
          >
