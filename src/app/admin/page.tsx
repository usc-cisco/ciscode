...
export default function Admin() {
  ...
  const [verifiedProblemCounts, setVerifiedProblemCounts] =
    useState<AdminCount>({
      count: 0,
      lastMonthCount: 0,
    });
  ...
  const [unverifiedProblemCounts, setUnverifiedProblemCounts] =
    useState<AdminCount>({
      count: 0,
      lastMonthCount: 0,
    });
  ...
  useEffect(() => {
    setCurrentAdminTable(
      (params.get("table") as AdminTableEnum) ||
        AdminTableEnum.VERIFIED_PROBLEMS,
    );
  }, [params]);
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid grid-rows-3 md:grid-cols-3 md:grid-rows-1 gap-2 mb-8">
          <AdminCard
            ...
            value={verifiedProblemCounts.count}
            lastMonth={verifiedProblemCounts.lastMonthCount}
            Icon={Code2}
            handleView={handleTableChange(AdminTableEnum.VERIFIED_PROBLEMS)}
          />
          <AdminCard
            title="Active Users"
            value={userCounts.count}
            lastMonth={userCounts.lastMonthCount}
            Icon={User}
            handleView={handleTableChange(AdminTableEnum.ACTIVE_USERS)}
          />
          <AdminCard
            title="Offered Problems"
            value={unverifiedProblemCounts.count}
            lastMonth={unverifiedProblemCounts.lastMonthCount}
            Icon={CodeIcon}
            handleView={handleTableChange(AdminTableEnum.OFFERED_PROBLEMS)}
          />
        </div>
